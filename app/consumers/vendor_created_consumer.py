...
def vendor_callback(ch, method, properties, body):
    try:
        ...
        db = SessionLocal()
        try:
            # Idempotency check: ensure the vendor doesn't already exist in the VendorModel
            existing_vendor = db.query(VendorModel).filter(VendorModel.id == vendor_data["id"]).first()
            if existing_vendor:
                logger.info(f"Vendor with id {vendor_data['id']} already exists in chatbot-service. Skipping insertion.")
            else:
                ...
                db.add(vendor_obj)
                db.commit()
                logger.info(f"Vendor added in chatbot-service: {vendor_obj.name} (ID: {vendor_obj.id})")
