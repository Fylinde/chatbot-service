...
def seller_callback(ch, method, properties, body):
    try:
        ...
        db = SessionLocal()
        try:
            # Idempotency check: ensure the seller doesn't already exist in the sellerModel
            existing_seller = db.query(SellerModel).filter(SellerModel.id == seller_data["id"]).first()
            if existing_seller:
                logger.info(f"seller with id {seller_data['id']} already exists in chatbot-service. Skipping insertion.")
            else:
                ...
                db.add(seller_obj)
                db.commit()
                logger.info(f"seller added in chatbot-service: {seller_obj.name} (ID: {seller_obj.id})")
...
def start_seller_consuming():
    ...
    logger.info(f"Declared queue {queue_name} and binding it to seller_events exchange")
    ...
    logger.info('Waiting for seller messages. To exit press CTRL+C')
